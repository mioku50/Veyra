...
2. **Improve the substance of findings.** *(largely addressed 2026-09-23; see [the substance of a card](audits/2026-09-23-nova-substance.md). A finding is stated against work already done, and a significant one carries proposed work rather than a topic. Whether that work is worth doing is unmeasured — that is item 1.)* Explain changes to a release, policy, integration, product or API rather than presenting a raw commit count as a headline. Show evidence links, observation/publication dates, what's new relative to the previous brief, relevance to the goal and a concrete next step. If evidence is incomplete, say so.
   ...
   Cards now carry both the publication date and the date Nova found them. They mark what is **new** or **re-read** since the owner's previous visit and link to the original. Every reading says how much of the article it stood on: the model is given the first 24 sentences, which was 24 of 32 for StableFX, from a text already cut to 6,000 characters. A paid endpoint whose seller rotates its payee address had produced a new card on every pass, 24 of Parallel search in eight days; it is now listed once. The page no longer receives article text, which was 310 KiB of a 770 KiB brief.

   Not done: giving the model the whole article. It was built, and a dry run appeared to show it losing every plan. The same run with the old input lost them too, and it had used the local environment's `ministral-8b` rather than production's reading model. Edition 7 waits for a measurement on that model. Found and fixed on the way: Circle moved its Nanopayments reference page, and the reader, which refused all redirects, would have dropped it from every reading. It was caught before any stored reading lost it, and the reader now follows one redirect within the same approved site.
...
5. **Feedback-to-brief loop.** *(verified and partly fixed 2026-09-22.)* Verify that explicit owner feedback changes future prioritization without silently suppressing safety alerts or whole publishers. Keep feedback failures visible and avoid claiming personalization without demonstrated effect.
   ...
   Not changed, and a known limit: "useful" stays category-wide. For an announcement-only owner it cannot tell a good Arc finding from cirBTC. Boosting on headline words is the inflation the scorer was built to refuse; the narrower positive lever is "follow". **Not yet demonstrated:** that any of this moved an actual brief. No personalization is claimed until an owner's ranking is observed to change.
