...
## Reading model, 2026-09-22 — OpenRouter, and the gap edition 6 left open

Edition 6 shipped with a cost recorded above: ministral-14b would not connect
"sponsored gas on Arc" to the owner's own confirmed "operational wallet не
выбран", six times out of six, reasoning that the owner had never written the
word "gas". Matching on words. The owner supplied an OpenRouter key, which is
not behind the Alibaba firewall that made agentrouter.org unusable.

Measured on the six hardest stored cards — the four the owner reported as
invented work, and the two ministral would not connect. Two runs each unless
noted.

| model | answered | wallet hop | invented work | goal's language |
| --- | --- | --- | --- | --- |
| deepseek-v4.1-flash | 10/12 | 2/3 | none | 8/10 |
| deepseek-v4-flash | 14/18 | 2/4 | one stretched `decides` | 2/14 |
| ministral-14b (was) | 15/15 | **0/6** | none | 15/15 |
| deepseek-v4-flash-0731 | 3/12 | — | — | — |
| mistral-small-24b | 11/12 | 0/4 | echoed these instructions as its answer | — |
| qwen3-30b-a3b | 11/12 | 2/4 | **severe** — see below | 11/11 |
| qwen3.5-flash-02-23 | 0/12 | — | — | — |

qwen3-30b-a3b is the useful negative result: it reproduced every defect
edition 6 exists to stop, unprompted. "Интегрировать cirBTC в Veyra как
источник обеспечения" from an asset launch; "Обновить протокол Veyra для
атомарных транзакций" from an FX launch; and an Arc Portal plan justifying
itself with the owner's statement "«Приоритет — полезность исследований Nova".
A structural gate stops a claim from being unfounded; it cannot stop a model
from finding a bad reason.

mistral-small-24b is the other kind. Asked what the event changed relative to
the project, it returned the rule text from the prompt: "ProjectState does not
mention FX or settlement, so check FX -- forbidden."

### Why calls were coming back empty, and the knob that fixed it

OpenRouter serves one model id from a dozen upstream providers and picks per
request. Several do not implement `response_format`, so the JSON instruction is
silently dropped, prose comes back, and the reading is thrown out — at random,
about a third of calls. A raw probe against the same model answered 3 of 3
with `finish_reason: stop`, which is what proved it was routing and not the
model.

`LLM_EXTRA_BODY` merges a router's own knob into the request body, and can
never displace the model, the messages or anything else this client sets.
`{"provider":{"require_parameters":true}}` took v4.1-flash from 8 of 12 to 10.

A second cause was real and separate: `LLM_MAX_COMPLETION_TOKENS` was one
constant for both workloads, sized for the one-sentence rewrite. A reasoning
model spends multiples of it on its trace before emitting any content, and a
budget that runs out mid-trace returns an empty message the caller can only
report as `invalid_response`. The reading call now asks for 12k, bounded by
the byte cap and the timeout either way.

### Settled

`deepseek/deepseek-v4.1-flash` on OpenRouter. It writes the best proposed work
of any candidate measured — a wallet checklist quoting EIP-3085, one merged
USDC balance, USDC fee labels, rejection of EIP-4844 type-3 — built on the
owner's own open decision rather than on what they never said.

Still unverified, and the one that matters: **whether OpenRouter answers a
Vercel function.** agentrouter.org answered this laptop and not the
deployment. Nothing here is evidence about production until a reading runs
there.

**Answered, 2026-09-23.** The scheduled pass at 06:04 UTC wrote two readings
from the deployment, each recorded as `OpenRouter · deepseek/deepseek-v4.1-flash`,
with no reading failures in the pass.
