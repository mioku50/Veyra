  ...
  return <section className="mx-auto grid max-w-6xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[0.9fr_1.1fr]">
    ...
    <Card><CardHeader><CardTitle>2. Evidence-backed result</CardTitle></CardHeader><CardContent>
      {!selection ? <div className="py-16 text-center text-muted-foreground"><Search className="mx-auto mb-3 size-8" /><p>No selection created yet.</p><p className="mt-1 text-sm">Discovery and ranking do not initiate payment or execution.</p></div> : <div className="space-y-5">
        ...
        <div className="flex flex-wrap gap-2">{selection.visibility === "public" ? <Button asChild variant="outline"><Link href={`/trust/selections/${selection.publicId}`}>View public receipt</Link></Button> : null}<Button variant="outline" onClick={() => void viewEvidence()} disabled={Boolean(busy)}>{busy === "evidence" ? <Loader2 className="animate-spin" /> : <Search />}View evidence</Button><Button variant="outline" onClick={() => void issueClearance()} disabled={Boolean(busy)}>{busy === "clearance" ? <Loader2 className="animate-spin" /> : <ShieldCheck />}Issue winner-bound clearance</Button><Button variant="outline" onClick={() => void publishProof()} disabled={Boolean(busy)}>{busy === "proof" ? <Loader2 className="animate-spin" /> : <BadgeCheck />}Publish optional Arc proof</Button></div>
