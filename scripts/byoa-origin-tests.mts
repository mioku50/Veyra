...
const moved = origins({ VERCEL_PROJECT_PRODUCTION_URL: "veyras.vercel.app" });
assert.ok(moved.has("https://veyras.vercel.app"), "the production host must be trusted");
...
const configured = origins({
  ...
  NEXT_PUBLIC_APP_URL: "https://veyras.vercel.app",
});
assert.ok(configured.has("https://veyra.app"));
assert.ok(configured.has("https://www.veyra.app"));
assert.ok(configured.has("https://veyras.vercel.app"));
...
assert.equal(moved.has("https://veyras.vercel.app.evil.example"), false);
...
const withJunk = origins({
  ...
  VERCEL_PROJECT_PRODUCTION_URL: "veyras.vercel.app",
});
assert.ok(withJunk.has("https://veyra.app"));
assert.ok(withJunk.has("https://veyras.vercel.app"));
